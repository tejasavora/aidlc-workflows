...
export function resolveProjectDir(explicitDir?: string): string {
  ...
  // 3. Script path derivation: strip .claude/tools from import.meta.dir
  const scriptDir = dirname(fileURLToPath(import.meta.url));
  const toolsSuffix = `${sep}.claude${sep}tools`;
  if (scriptDir.endsWith(toolsSuffix)) {
    return scriptDir.slice(0, -toolsSuffix.length);
  }

  // 4. CWD has .claude directory
...
export function resolveProjectDirFromHook(importMetaUrl: string): string {
  ...
  // 2. Script path derivation: strip .claude/hooks from the hook's location
  const scriptDir = dirname(fileURLToPath(importMetaUrl));
  const hooksSuffix = `${sep}.claude${sep}hooks`;
  if (scriptDir.endsWith(hooksSuffix)) {
    return scriptDir.slice(0, -hooksSuffix.length);
  }

  // 3. CWD has .claude directory
  const cwd = process.cwd();
  if (existsSync(join(cwd, ".claude"))) {
    return cwd;
  }
...
export function loadScopeMapping(): Record<string, ScopeDefinition> {
  ...
  // Lazy require to avoid the circular import (aidlc-graph imports from us).
  const { loadScopeGrid } = require("./aidlc-graph.ts") as {
    loadScopeGrid: () => Record<string, { stages: Record<string, "EXECUTE" | "SKIP"> }>;
  };
