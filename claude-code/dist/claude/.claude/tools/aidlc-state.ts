...
function handleAdvance(args: string[]): void {
  ...
  // Idempotency guard — if completedSlug is already [x] AND nextSlug is
  // already [-] with Current Stage pointing at it, this is a replay. Skip
  // the whole emission block and exit cleanly, rather than doubling
...
function handleGateStart(args: string[]): void {
  if (args.length < 1) error("Usage: aidlc-state.ts gate-start <slug> [--artifacts <csv>]");
  const slug = args[0];
...
function handlePracticesPromote(args: string[]): void {
  ...
  const targetRoot = flags["target-dir"] ?? join(pd, ".claude");
  const teamMdPath = join(targetRoot, "rules", "aidlc-team.md");
  const guardrailsPath = join(targetRoot, "rules", "aidlc-project.md");
