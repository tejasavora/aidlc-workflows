...
description: |
  Execute a DR drill: trigger failover, measure actual RTO/RPO, verify recovery.
  Self-healing: if RTO exceeds target → diagnose → fix (replicate, pre-warm) → re-drill.
  Always requires human approval before triggering failover.
metadata:
  phase: operations
  stage: dr-validation
  per-unit: "false"
