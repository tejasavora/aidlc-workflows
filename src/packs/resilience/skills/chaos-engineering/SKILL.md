...
description: |
  Inject failures during load to verify the system degrades gracefully and recovers.
  Tool-agnostic: AWS FIS, Litmus, Gremlin, tc/kill scripts, or custom. Requires human
  approval before each experiment. Verifies steady state before and after each injection.
metadata:
  phase: operations
