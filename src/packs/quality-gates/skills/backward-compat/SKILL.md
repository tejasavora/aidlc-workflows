...
description: |
  Detects breaking changes in API contracts between the current version and the previous
  committed version. Compares API specs (OpenAPI, protobuf, GraphQL) using git diff and
  tool-agnostic comparison. Categorises changes as breaking, deprecation, or addition.
  No auto-fix — breaking changes are business decisions. All findings presented to human.
metadata:
  phase: construction
  stage: backward-compat
  per-unit: "true"
