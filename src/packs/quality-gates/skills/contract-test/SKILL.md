---
name: aidlc-contract-test
description: |
  Consumer-driven contract testing to prevent API breaking changes between services.
  Asks which services are consumers and where contracts are defined. Verifies each
  consumer-provider pair. Self-healing: if provider violates a contract, checks if
  provider code diverged from its design and fixes the code to match.
  Tool-agnostic: Pact, Spring Cloud Contract, Prism, or custom.
metadata:
  phase: construction
  stage: contract-test
  per-unit: "false"
