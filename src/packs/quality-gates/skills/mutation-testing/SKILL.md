---
name: aidlc-mutation-testing
description: |
  Verifies test quality by introducing small code mutations and checking if existing
  tests catch them. Calculates a mutation score. Self-healing: if score is below
  threshold, identifies survived mutants and generates targeted tests to kill them.
  Re-runs to confirm improvement. Tool-agnostic: mutmut, Stryker, PIT, or custom.
metadata:
  phase: construction
