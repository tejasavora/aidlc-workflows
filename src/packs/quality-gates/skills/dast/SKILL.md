...
name: aidlc-dast
description: |
  Dynamic Application Security Testing — runs a web scanner (OWASP ZAP, Burp Suite,
  Nuclei, or configured tool) against the deployed staging environment. Produces a
  vulnerability report with CWE IDs, severity, and reproduction steps. ALL findings
  presented to human. No auto-fix — dynamic security findings require human judgment.
  Maps to PCI-DSS Req 11.3 for compliance-evidence.
metadata:
  phase: operations
  stage: dast
  per-unit: "false"
