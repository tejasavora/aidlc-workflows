---
name: aidlc-visual-regression
description: |
  Screenshot diff for UI changes. Captures screenshots of key pages and components,
  compares against baseline, and presents side-by-side diffs with percentage change.
  Human decides to approve new baseline or reject (fix CSS/layout). Tool-agnostic:
  Percy, Chromatic, BackstopJS, Playwright + pixelmatch, or custom.
  Only activates when project has a frontend AND visual_regression is configured.
metadata:
  phase: construction
