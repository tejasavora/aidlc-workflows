...
name: aidlc-e2e-test
description: |
  Run full end-to-end tests (Playwright, Cypress, Selenium, or project-configured
  framework) against the running application. Pre-deploy quality gate: catches
  integration regressions before human review. Self-healing loop: fail → diagnose
  (flaky? real regression? environment?) → fix flaky/env issues → re-run → escalate
  if real regression.
metadata:
  phase: construction
  stage: e2e-test
