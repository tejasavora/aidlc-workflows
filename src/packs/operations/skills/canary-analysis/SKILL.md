...
description: |
  Monitor error rate and latency deltas during canary or blue-green deployments.
  Compares canary metrics against baseline for a configurable analysis window.
  Auto-promotes if all metrics are within thresholds; auto-rolls-back if any threshold
  is breached. For production, presents the promotion/rollback decision to human before
  executing. Tool-agnostic: reads monitoring config from toolchain.yaml.
metadata:
  phase: operations
