...
description: |
  Design disaster recovery architecture: backup strategy, replication topology,
  and RTO/RPO-driven architecture selection (active-active, hot-standby, warm,
  backup-restore). Includes decision matrix. Requires human confirmation of
  RTO/RPO targets before architecture is selected.
metadata:
  phase: construction
  stage: dr-design
  per-unit: "false"
