...
description: >
  Run the AI-DLC `e2e-test` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage e2e-test --single`:
  the engine emits one run-stage directive for e2e-test and its gate, the
...
# AI-DLC Stage Runner — e2e-test

Run the `e2e-test` stage on its own. This is opt-in packaging over
`/aidlc --stage e2e-test --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage e2e-test --single
   ```

   The engine emits one `run-stage` directive for `e2e-test` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage e2e-test --result completed
   ```
