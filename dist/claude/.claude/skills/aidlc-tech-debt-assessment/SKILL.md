---
name: aidlc-tech-debt-assessment
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `tech-debt-assessment` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage tech-debt-assessment --single`:
  the engine emits one run-stage directive for tech-debt-assessment and its gate, the
...
# AI-DLC Stage Runner — tech-debt-assessment

Run the `tech-debt-assessment` stage on its own. This is opt-in packaging over
`/aidlc --stage tech-debt-assessment --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage tech-debt-assessment --single
   ```

   The engine emits one `run-stage` directive for `tech-debt-assessment` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage tech-debt-assessment --result completed
   ```
