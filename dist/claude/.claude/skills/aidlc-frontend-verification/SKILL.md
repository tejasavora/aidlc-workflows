---
name: aidlc-frontend-verification
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `frontend-verification` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage frontend-verification --single`:
  the engine emits one run-stage directive for frontend-verification and its gate, the
...
# AI-DLC Stage Runner — frontend-verification

Run the `frontend-verification` stage on its own. This is opt-in packaging over
`/aidlc --stage frontend-verification --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage frontend-verification --single
   ```

   The engine emits one `run-stage` directive for `frontend-verification` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage frontend-verification --result completed
   ```
