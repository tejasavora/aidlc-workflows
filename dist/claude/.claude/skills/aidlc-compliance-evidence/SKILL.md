...
description: >
  Run the AI-DLC `compliance-evidence` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage compliance-evidence --single`:
  the engine emits one run-stage directive for compliance-evidence and its gate, the
...
# AI-DLC Stage Runner — compliance-evidence

Run the `compliance-evidence` stage on its own. This is opt-in packaging over
`/aidlc --stage compliance-evidence --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage compliance-evidence --single
   ```

   The engine emits one `run-stage` directive for `compliance-evidence` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage compliance-evidence --result completed
   ```
