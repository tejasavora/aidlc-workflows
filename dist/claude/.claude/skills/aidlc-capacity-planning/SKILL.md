---
name: aidlc-capacity-planning
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `capacity-planning` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage capacity-planning --single`:
  the engine emits one run-stage directive for capacity-planning and its gate, the
...
# AI-DLC Stage Runner — capacity-planning

Run the `capacity-planning` stage on its own. This is opt-in packaging over
`/aidlc --stage capacity-planning --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage capacity-planning --single
   ```

   The engine emits one `run-stage` directive for `capacity-planning` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage capacity-planning --result completed
   ```
