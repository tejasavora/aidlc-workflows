...
description: >
  Run the AI-DLC `data-migration` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage data-migration --single`:
  the engine emits one run-stage directive for data-migration and its gate, the
...
# AI-DLC Stage Runner — data-migration

Run the `data-migration` stage on its own. This is opt-in packaging over
`/aidlc --stage data-migration --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage data-migration --single
   ```

   The engine emits one `run-stage` directive for `data-migration` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage data-migration --result completed
   ```
