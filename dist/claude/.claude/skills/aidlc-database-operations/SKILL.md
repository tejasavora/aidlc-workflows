---
name: aidlc-database-operations
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `database-operations` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage database-operations --single`:
  the engine emits one run-stage directive for database-operations and its gate, the
...
# AI-DLC Stage Runner — database-operations

Run the `database-operations` stage on its own. This is opt-in packaging over
`/aidlc --stage database-operations --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage database-operations --single
   ```

   The engine emits one `run-stage` directive for `database-operations` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage database-operations --result completed
   ```
