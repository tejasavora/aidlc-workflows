---
name: aidlc-access-control-review
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `access-control-review` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage access-control-review --single`:
  the engine emits one run-stage directive for access-control-review and its gate, the
...
# AI-DLC Stage Runner — access-control-review

Run the `access-control-review` stage on its own. This is opt-in packaging over
`/aidlc --stage access-control-review --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage access-control-review --single
   ```

   The engine emits one `run-stage` directive for `access-control-review` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage access-control-review --result completed
   ```
