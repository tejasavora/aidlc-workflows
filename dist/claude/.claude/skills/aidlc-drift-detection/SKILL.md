...
description: >
  Run the AI-DLC `drift-detection` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage drift-detection --single`:
  the engine emits one run-stage directive for drift-detection and its gate, the
...
# AI-DLC Stage Runner — drift-detection

Run the `drift-detection` stage on its own. This is opt-in packaging over
`/aidlc --stage drift-detection --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage drift-detection --single
   ```

   The engine emits one `run-stage` directive for `drift-detection` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage drift-detection --result completed
   ```
