---
name: aidlc-iac-execution
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `iac-execution` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage iac-execution --single`:
  the engine emits one run-stage directive for iac-execution and its gate, the
...
# AI-DLC Stage Runner — iac-execution

Run the `iac-execution` stage on its own. This is opt-in packaging over
`/aidlc --stage iac-execution --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage iac-execution --single
   ```

   The engine emits one `run-stage` directive for `iac-execution` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage iac-execution --result completed
   ```
