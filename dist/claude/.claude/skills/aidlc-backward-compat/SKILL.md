---
name: aidlc-backward-compat
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `backward-compat` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage backward-compat --single`:
  the engine emits one run-stage directive for backward-compat and its gate, the
...
# AI-DLC Stage Runner — backward-compat

Run the `backward-compat` stage on its own. This is opt-in packaging over
`/aidlc --stage backward-compat --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage backward-compat --single
   ```

   The engine emits one `run-stage` directive for `backward-compat` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage backward-compat --result completed
   ```
