---
name: aidlc-workflow-telemetry
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `workflow-telemetry` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage workflow-telemetry --single`:
  the engine emits one run-stage directive for workflow-telemetry and its gate, the
...
# AI-DLC Stage Runner — workflow-telemetry

Run the `workflow-telemetry` stage on its own. This is opt-in packaging over
`/aidlc --stage workflow-telemetry --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage workflow-telemetry --single
   ```

   The engine emits one `run-stage` directive for `workflow-telemetry` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage workflow-telemetry --result completed
   ```
