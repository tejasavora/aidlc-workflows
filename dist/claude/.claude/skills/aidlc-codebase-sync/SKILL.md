---
name: aidlc-codebase-sync
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `codebase-sync` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage codebase-sync --single`:
  the engine emits one run-stage directive for codebase-sync and its gate, the
...
# AI-DLC Stage Runner — codebase-sync

Run the `codebase-sync` stage on its own. This is opt-in packaging over
`/aidlc --stage codebase-sync --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage codebase-sync --single
   ```

   The engine emits one `run-stage` directive for `codebase-sync` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage codebase-sync --result completed
   ```
