...
description: >
  Run the AI-DLC `cost-estimation` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage cost-estimation --single`:
  the engine emits one run-stage directive for cost-estimation and its gate, the
...
# AI-DLC Stage Runner — cost-estimation

Run the `cost-estimation` stage on its own. This is opt-in packaging over
`/aidlc --stage cost-estimation --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage cost-estimation --single
   ```

   The engine emits one `run-stage` directive for `cost-estimation` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage cost-estimation --result completed
   ```
