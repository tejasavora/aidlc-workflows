---
name: aidlc-data-seeding
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `data-seeding` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage data-seeding --single`:
  the engine emits one run-stage directive for data-seeding and its gate, the
...
# AI-DLC Stage Runner — data-seeding

Run the `data-seeding` stage on its own. This is opt-in packaging over
`/aidlc --stage data-seeding --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage data-seeding --single
   ```

   The engine emits one `run-stage` directive for `data-seeding` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage data-seeding --result completed
   ```
