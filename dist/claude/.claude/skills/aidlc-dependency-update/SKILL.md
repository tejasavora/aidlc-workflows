...
description: >
  Run the AI-DLC `dependency-update` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage dependency-update --single`:
  the engine emits one run-stage directive for dependency-update and its gate, the
...
# AI-DLC Stage Runner — dependency-update

Run the `dependency-update` stage on its own. This is opt-in packaging over
`/aidlc --stage dependency-update --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage dependency-update --single
   ```

   The engine emits one `run-stage` directive for `dependency-update` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage dependency-update --result completed
   ```
