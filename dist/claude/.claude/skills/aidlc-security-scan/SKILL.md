...
description: >
  Run the AI-DLC `security-scan` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage security-scan --single`:
  the engine emits one run-stage directive for security-scan and its gate, the
...
# AI-DLC Stage Runner — security-scan

Run the `security-scan` stage on its own. This is opt-in packaging over
`/aidlc --stage security-scan --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage security-scan --single
   ```

   The engine emits one `run-stage` directive for `security-scan` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage security-scan --result completed
   ```
