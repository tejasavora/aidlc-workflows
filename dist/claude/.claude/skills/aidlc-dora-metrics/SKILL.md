...
description: >
  Run the AI-DLC `dora-metrics` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage dora-metrics --single`:
  the engine emits one run-stage directive for dora-metrics and its gate, the
...
# AI-DLC Stage Runner — dora-metrics

Run the `dora-metrics` stage on its own. This is opt-in packaging over
`/aidlc --stage dora-metrics --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage dora-metrics --single
   ```

   The engine emits one `run-stage` directive for `dora-metrics` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage dora-metrics --result completed
   ```
