---
name: aidlc-api-governance
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `api-governance` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage api-governance --single`:
  the engine emits one run-stage directive for api-governance and its gate, the
...
# AI-DLC Stage Runner — api-governance

Run the `api-governance` stage on its own. This is opt-in packaging over
`/aidlc --stage api-governance --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage api-governance --single
   ```

   The engine emits one `run-stage` directive for `api-governance` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage api-governance --result completed
   ```
