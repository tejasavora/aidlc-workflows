---
name: aidlc-dr-validation
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `dr-validation` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage dr-validation --single`:
  the engine emits one run-stage directive for dr-validation and its gate, the
...
# AI-DLC Stage Runner — dr-validation

Run the `dr-validation` stage on its own. This is opt-in packaging over
`/aidlc --stage dr-validation --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage dr-validation --single
   ```

   The engine emits one `run-stage` directive for `dr-validation` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage dr-validation --result completed
   ```
