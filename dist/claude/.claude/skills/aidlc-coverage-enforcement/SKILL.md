---
name: aidlc-coverage-enforcement
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `coverage-enforcement` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage coverage-enforcement --single`:
  the engine emits one run-stage directive for coverage-enforcement and its gate, the
...
# AI-DLC Stage Runner — coverage-enforcement

Run the `coverage-enforcement` stage on its own. This is opt-in packaging over
`/aidlc --stage coverage-enforcement --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage coverage-enforcement --single
   ```

   The engine emits one `run-stage` directive for `coverage-enforcement` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage coverage-enforcement --result completed
   ```
