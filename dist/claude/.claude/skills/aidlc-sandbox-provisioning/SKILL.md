---
name: aidlc-sandbox-provisioning
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `sandbox-provisioning` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage sandbox-provisioning --single`:
  the engine emits one run-stage directive for sandbox-provisioning and its gate, the
...
# AI-DLC Stage Runner — sandbox-provisioning

Run the `sandbox-provisioning` stage on its own. This is opt-in packaging over
`/aidlc --stage sandbox-provisioning --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage sandbox-provisioning --single
   ```

   The engine emits one `run-stage` directive for `sandbox-provisioning` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage sandbox-provisioning --result completed
   ```
