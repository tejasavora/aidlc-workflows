---
name: aidlc-sandbox-deploy
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `sandbox-deploy` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage sandbox-deploy --single`:
  the engine emits one run-stage directive for sandbox-deploy and its gate, the
...
# AI-DLC Stage Runner — sandbox-deploy

Run the `sandbox-deploy` stage on its own. This is opt-in packaging over
`/aidlc --stage sandbox-deploy --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage sandbox-deploy --single
   ```

   The engine emits one `run-stage` directive for `sandbox-deploy` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage sandbox-deploy --result completed
   ```
