---
name: aidlc-static-analysis
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `static-analysis` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage static-analysis --single`:
  the engine emits one run-stage directive for static-analysis and its gate, the
...
# AI-DLC Stage Runner — static-analysis

Run the `static-analysis` stage on its own. This is opt-in packaging over
`/aidlc --stage static-analysis --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage static-analysis --single
   ```

   The engine emits one `run-stage` directive for `static-analysis` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage static-analysis --result completed
   ```
