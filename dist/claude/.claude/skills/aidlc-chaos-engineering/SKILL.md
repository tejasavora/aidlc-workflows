---
name: aidlc-chaos-engineering
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `chaos-engineering` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage chaos-engineering --single`:
  the engine emits one run-stage directive for chaos-engineering and its gate, the
...
# AI-DLC Stage Runner — chaos-engineering

Run the `chaos-engineering` stage on its own. This is opt-in packaging over
`/aidlc --stage chaos-engineering --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage chaos-engineering --single
   ```

   The engine emits one `run-stage` directive for `chaos-engineering` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage chaos-engineering --result completed
   ```
