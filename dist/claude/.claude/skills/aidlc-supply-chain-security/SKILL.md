---
name: aidlc-supply-chain-security
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `supply-chain-security` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage supply-chain-security --single`:
  the engine emits one run-stage directive for supply-chain-security and its gate, the
...
# AI-DLC Stage Runner — supply-chain-security

Run the `supply-chain-security` stage on its own. This is opt-in packaging over
`/aidlc --stage supply-chain-security --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage supply-chain-security --single
   ```

   The engine emits one `run-stage` directive for `supply-chain-security` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage supply-chain-security --result completed
   ```
