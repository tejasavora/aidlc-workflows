---
name: aidlc-cost-governance
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `cost-governance` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage cost-governance --single`:
  the engine emits one run-stage directive for cost-governance and its gate, the
...
# AI-DLC Stage Runner — cost-governance

Run the `cost-governance` stage on its own. This is opt-in packaging over
`/aidlc --stage cost-governance --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage cost-governance --single
   ```

   The engine emits one `run-stage` directive for `cost-governance` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage cost-governance --result completed
   ```
