...
description: >
  Run the AI-DLC `postmortem` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage postmortem --single`:
  the engine emits one run-stage directive for postmortem and its gate, the
...
# AI-DLC Stage Runner — postmortem

Run the `postmortem` stage on its own. This is opt-in packaging over
`/aidlc --stage postmortem --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage postmortem --single
   ```

   The engine emits one `run-stage` directive for `postmortem` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage postmortem --result completed
   ```
