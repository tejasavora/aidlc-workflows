---
name: aidlc-ha-design
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `ha-design` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage ha-design --single`:
  the engine emits one run-stage directive for ha-design and its gate, the
...
# AI-DLC Stage Runner — ha-design

Run the `ha-design` stage on its own. This is opt-in packaging over
`/aidlc --stage ha-design --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage ha-design --single
   ```

   The engine emits one `run-stage` directive for `ha-design` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage ha-design --result completed
   ```
