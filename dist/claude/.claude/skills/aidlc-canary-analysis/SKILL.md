---
name: aidlc-canary-analysis
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `canary-analysis` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage canary-analysis --single`:
  the engine emits one run-stage directive for canary-analysis and its gate, the
...
# AI-DLC Stage Runner — canary-analysis

Run the `canary-analysis` stage on its own. This is opt-in packaging over
`/aidlc --stage canary-analysis --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage canary-analysis --single
   ```

   The engine emits one `run-stage` directive for `canary-analysis` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage canary-analysis --result completed
   ```
