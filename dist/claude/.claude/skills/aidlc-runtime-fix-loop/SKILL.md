---
name: aidlc-runtime-fix-loop
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `runtime-fix-loop` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage runtime-fix-loop --single`:
  the engine emits one run-stage directive for runtime-fix-loop and its gate, the
...
# AI-DLC Stage Runner — runtime-fix-loop

Run the `runtime-fix-loop` stage on its own. This is opt-in packaging over
`/aidlc --stage runtime-fix-loop --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage runtime-fix-loop --single
   ```

   The engine emits one `run-stage` directive for `runtime-fix-loop` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage runtime-fix-loop --result completed
   ```
