---
name: aidlc-contract-generation
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `contract-generation` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage contract-generation --single`:
  the engine emits one run-stage directive for contract-generation and its gate, the
...
# AI-DLC Stage Runner — contract-generation

Run the `contract-generation` stage on its own. This is opt-in packaging over
`/aidlc --stage contract-generation --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage contract-generation --single
   ```

   The engine emits one `run-stage` directive for `contract-generation` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage contract-generation --result completed
   ```
