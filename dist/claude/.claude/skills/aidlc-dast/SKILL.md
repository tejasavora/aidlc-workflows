...
description: >
  Run the AI-DLC `dast` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage dast --single`:
  the engine emits one run-stage directive for dast and its gate, the
...
# AI-DLC Stage Runner — dast

Run the `dast` stage on its own. This is opt-in packaging over
`/aidlc --stage dast --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage dast --single
   ```

   The engine emits one `run-stage` directive for `dast` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage dast --result completed
   ```
