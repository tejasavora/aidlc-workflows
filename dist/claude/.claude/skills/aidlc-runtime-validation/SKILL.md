---
name: aidlc-runtime-validation
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `runtime-validation` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage runtime-validation --single`:
  the engine emits one run-stage directive for runtime-validation and its gate, the
...
# AI-DLC Stage Runner — runtime-validation

Run the `runtime-validation` stage on its own. This is opt-in packaging over
`/aidlc --stage runtime-validation --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage runtime-validation --single
   ```

   The engine emits one `run-stage` directive for `runtime-validation` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage runtime-validation --result completed
   ```
