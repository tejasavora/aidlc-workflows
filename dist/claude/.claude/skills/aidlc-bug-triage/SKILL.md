...
description: >
  Run the AI-DLC `bug-triage` stage (operation phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage bug-triage --single`:
  the engine emits one run-stage directive for bug-triage and its gate, the
...
# AI-DLC Stage Runner — bug-triage

Run the `bug-triage` stage on its own. This is opt-in packaging over
`/aidlc --stage bug-triage --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage bug-triage --single
   ```

   The engine emits one `run-stage` directive for `bug-triage` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage bug-triage --result completed
   ```
