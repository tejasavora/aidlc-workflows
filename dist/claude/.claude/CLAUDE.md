...
- **Skill**: `.claude/skills/aidlc/` — Orchestrator (`SKILL.md`), stage protocol, and 78 stage files across 5 phase directories
...
- **Agents**: `.claude/agents/` — 15 agents: 11 domain-expert personas (product, design, delivery, architect, aws-platform, compliance, devsecops, developer, quality, pipeline-deploy, operations), 3 review-only agents (product-lead, architecture-reviewer, stage-reviewer), and the adaptive-workflows composer. Each is a flat `.md` file prefixed `aidlc-<role>-agent.md`; the conductor adopts domain personas inline where the stage mode calls for it and delegates workers for the four dispatched stages (2.1 pipeline, 2.2 subagent, 2.4 mob, 3.5 subagent), reviewer passes, and composer requests via the `Task` tool.
