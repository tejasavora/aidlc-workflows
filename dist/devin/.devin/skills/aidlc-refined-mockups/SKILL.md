...
description: >
  ...
  the engine emits one run-stage directive for refined-mockups and its gate, the
...
# AI-DLC Stage Runner — refined-mockups

Run the `refined-mockups` stage on its own. This is opt-in packaging over
`/aidlc --stage refined-mockups --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts next --stage refined-mockups --single
   ```

   The engine emits one `run-stage` directive for `refined-mockups` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts report --single --stage refined-mockups --result completed
   ```
