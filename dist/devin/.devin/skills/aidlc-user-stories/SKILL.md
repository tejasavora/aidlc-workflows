...
description: >
  ...
  the engine emits one run-stage directive for user-stories and its gate, the
...
Run the `user-stories` stage on its own. This is opt-in packaging over
`/aidlc --stage user-stories --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts next --stage user-stories --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts report --single --stage user-stories --result completed
   ```
