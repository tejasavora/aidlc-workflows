...
description: >
  ...
  the engine emits one run-stage directive for functional-design and its gate, the
...
`/aidlc --stage functional-design --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts next --stage functional-design --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts report --single --stage functional-design --result completed
   ```
