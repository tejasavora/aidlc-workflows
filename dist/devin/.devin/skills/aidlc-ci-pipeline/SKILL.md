...
description: >
  ...
  the engine emits one run-stage directive for ci-pipeline and its gate, the
...
# AI-DLC Stage Runner — ci-pipeline

Run the `ci-pipeline` stage on its own. This is opt-in packaging over
`/aidlc --stage ci-pipeline --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts next --stage ci-pipeline --single
   ```

   The engine emits one `run-stage` directive for `ci-pipeline` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts report --single --stage ci-pipeline --result completed
   ```
