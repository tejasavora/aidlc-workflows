...
description: >
  ...
  the engine emits one run-stage directive for nfr-design and its gate, the
...
# AI-DLC Stage Runner — nfr-design

Run the `nfr-design` stage on its own. This is opt-in packaging over
`/aidlc --stage nfr-design --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts next --stage nfr-design --single
   ```

   The engine emits one `run-stage` directive for `nfr-design` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts report --single --stage nfr-design --result completed
   ```
