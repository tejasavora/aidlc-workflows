...
description: >
  ...
  the engine emits one run-stage directive for team-formation and its gate, the
...
Run the `team-formation` stage on its own. This is opt-in packaging over
`/aidlc --stage team-formation --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts next --stage team-formation --single
   ```

   The engine emits one `run-stage` directive for `team-formation` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts report --single --stage team-formation --result completed
   ```
