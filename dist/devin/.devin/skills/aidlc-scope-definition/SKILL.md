...
description: >
  ...
  the engine emits one run-stage directive for scope-definition and its gate, the
...
# AI-DLC Stage Runner — scope-definition

Run the `scope-definition` stage on its own. This is opt-in packaging over
`/aidlc --stage scope-definition --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts next --stage scope-definition --single
   ```

   The engine emits one `run-stage` directive for `scope-definition` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts report --single --stage scope-definition --result completed
   ```
