...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts next --stage nfr-requirements --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts report --single --stage nfr-requirements --result completed
   ```
