---
name: aidlc-domain-design
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC `domain-design` stage (inception phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage domain-design --single`:
  the engine emits one run-stage directive for domain-design and its gate, the
...
# AI-DLC Stage Runner — domain-design

Run the `domain-design` stage on its own. This is opt-in packaging over
`/aidlc --stage domain-design --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts next --stage domain-design --single
   ```

   The engine emits one `run-stage` directive for `domain-design` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .devin/tools/aidlc-orchestrate.ts report --single --stage domain-design --result completed
   ```
