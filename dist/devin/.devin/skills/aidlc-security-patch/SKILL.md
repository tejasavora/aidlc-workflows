...
1. `directive = bun .devin/tools/aidlc-orchestrate.ts next --scope security-patch $ARGUMENTS`
...
3. `bun .devin/tools/aidlc-orchestrate.ts report --stage <directive.stage> --result <outcome> [--user-input "<text>"]` when the directive names a stage; omit `--stage` only for non-stage report round-trips.
...
- **Default to CONTINUATION.** Treat the input as new-work ONLY when it clearly
  ...
  (`bun .devin/tools/aidlc-utility.ts intent --json` gives its `slug` and
...
- **On CONFIRM**, re-run `next` with `--new-intent`, the confirmed scope, and the
  ...
  ```bash
  bun .devin/tools/aidlc-orchestrate.ts next --new-intent --scope <the confirmed scope> "<the new-work description>"
  ```
