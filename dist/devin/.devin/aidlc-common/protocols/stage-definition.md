...
under `.devin/aidlc-common/stages/`. The schema (`stage-schema.ts`), the
...
`.devin/tools/data/stage-graph.json` from the YAML sources; the runtime
...
| `lead_agent` | string | yes | agent slug; validated dynamically against `.devin/agents/*.md` via `loadAgents()` — no hardcoded enum |
...
`.devin/tools/data/stage-graph.json`. Consumers continue to read the compiled
