...
```bash
bun .devin/tools/aidlc-state.ts practices-event \
  --type discovered \
...
1. Open the gate before the question:
   `bun .devin/tools/aidlc-orchestrate.ts report --stage
...
```bash
bun .devin/tools/aidlc-state.ts practices-promote \
...
- a new `.devin/sensors/aidlc-<id>.md` manifest for a verification
