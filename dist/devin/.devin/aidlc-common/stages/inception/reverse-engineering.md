...
`bun .devin/tools/aidlc-orchestrate.ts report --stage reverse-engineering --result skipped --reason "<reason>"`.
...
```
bun .devin/tools/aidlc-utility.ts codekb-scope-diff --repo <repo>
```
...
- If every repo is reused on an ordinary workflow run, report the stage as
  ...
  `bun .devin/tools/aidlc-orchestrate.ts report --stage reverse-engineering --result skipped --reason "codekb reuse: all resolved stores CURRENT, human chose reuse"`.
...
Template in
`.devin/knowledge/aidlc-developer-agent/re-artifacts.md`.
...
9. **reverse-engineering-timestamp.md** - Records when reverse engineering was performed (date, commit hash if available) and MUST end with the structured `## Scope of Analysis` block from the re-artifacts.md template, filled from the developer's Scan Coverage - what the run ACTUALLY analyzed deeply, not what was aspired to. This is the freshness/staleness marker the Step 1 rerun guard reads. For the block's `fingerprint:` line, run the mint command with the analyzed paths (comma-separated) and paste its output verbatim:
   ...
   ```
   bun .devin/tools/aidlc-utility.ts codekb-scope-diff --repo <repo> --mint --paths <analyzed paths>
   ```
...
```
bun .devin/tools/aidlc-utility.ts codekb-path --repo <repo>
```
...
```
bun .devin/tools/aidlc-utility.ts codekb-scope-diff --repo <repo> --compare <record>/inception/reverse-engineering/scope-draft-<repo>.md
```
...
`bun .devin/tools/aidlc-orchestrate.ts report --stage reverse-engineering --result <outcome>`.
