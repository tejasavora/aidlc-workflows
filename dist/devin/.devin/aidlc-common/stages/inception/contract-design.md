...
Load aidlc-aws-platform-agent persona from `agents/aidlc-aws-platform-agent.md` and knowledge from `.devin/knowledge/aidlc-aws-platform-agent/` for integration-mechanism awareness (sync REST vs. async messaging vs. shared store).
...
`bun .devin/tools/aidlc-orchestrate.ts report --stage contract-design --result <outcome>`.
