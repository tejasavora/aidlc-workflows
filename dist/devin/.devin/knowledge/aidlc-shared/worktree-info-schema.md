...
```
bun .devin/tools/aidlc-worktree.ts info --slug <kebab-slug>
```
...
- Implementation: `.devin/tools/aidlc-worktree.ts` (`handleInfo` handler)
- Test: `tests/unit/t72-worktree-info.sh`
- Caller: `.devin/skills/aidlc/SKILL.md` (per-Bolt-loop halt-and-ask flow)
...
- Audit-format spec: `.devin/knowledge/aidlc-shared/audit-format.md` `WORKTREE_CREATED` row
